...
<li @class([
    ...
    <a href="{{ $url }}" @if ($shouldOpenUrlInNewTab) target="_blank" @endif
        x-on:click="window.matchMedia(`(max-width: 1024px)`).matches && $store.sidebar.close()" @class([
    ...
    'bg-gray-100 dark:bg-white/5' => $active,
        ...
        <span @if (filament()->isSidebarCollapsibleOnDesktop()) x-show="true"
            x-transition:enter="lg:transition lg:delay-100" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" @endif @class([
